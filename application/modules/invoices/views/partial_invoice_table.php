...
<div id="DataTables_invoices_partial_wrapper" class="dataTables_wrapper form-inline dt-bootstrap no-footer table-responsive">
    <div class="col-sm-12">

        <table ui-jq="dataTable" class="table table-striped m-b-none dataTable no-footer" id="DataTablesinvoices_partial"
               role="grid" aria-describedby="DataTables_invoices_partial_info" data-order='[[ 0, "desc" ]]'
               ui-options="{
            ...
            <thead>
            <tr role="row">
                <th class="sorting" tabindex="0" aria-controls="DataTables_invoices_partial"
                ...
                <th class="sorting" tabindex="0" aria-controls="DataTables_invoices_partial" rowspan="1"
            ...
            foreach ($invoices as $invoice) {
                ...
                ?>
                <?php $invoiceIsRecurring = $this->db->get_where('ip_invoices_recurring', ['invoice_id' => $invoice->invoice_id])->num_rows() > 0; ?>
                <?php $tdStyle = $invoiceIsRecurring == true ? 'style="background-color: #D9E6FC !important"' : null; ?>
                <?php if ($this->db->get_where('ip_invoice_items', ['invoice_id' => $invoice->invoice_id])->num_rows() > 0) { ?>
                    <tr role="row" class="<?= $class; ?>">
                        ...
                        <td <?=$tdStyle; ?> class="amount <?php if ($invoice->invoice_sign == '-1') { echo 'text-danger'; }; ?>">
                            <?= format_currency($invoice->invoice_total); ?>
                        </td>

                        <td <?=$tdStyle; ?> class="amount">
                            <?= format_currency(-abs($invoice->invoice_balance)); ?>
                        </td>

                        <td <?=$tdStyle; ?> width="1% " style="width: 1px !important">
                            <div class="options btn-group">
                                ...
                                <ul class="dropdown-menu">
                                    ...
                                    <?php endif; ?>
                                    <li>
                                        <a href="<?= "/invoices/generate_pdf/{$invoice->invoice_id}"; ?>"
                                           target="_blank">
