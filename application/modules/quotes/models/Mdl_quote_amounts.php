...
class Mdl_quote_amounts extends CI_Model
{
    public function calculate($quoteId)
    {
        ...
        $quoteTotal = $this->calculate_discount($quoteId, $quoteSubtotal);
        ...
        $quoteDiscounts = $query->row();
        if ($quoteDiscounts->quote_discount_amount > 0) {
            $discountAmount = $quoteDiscounts->invoice_discount_amount;
        } elseif ($quoteDiscounts->quote_discount_percent > 0) {
            $discountAmount = $quoteTotal * $quoteDiscounts->invoice_discount_percent / 100;
        ...
        if ($this->db->get('ip_quote_amounts')->num_rows()) {
            $this->db->where('quote_id', $quoteId);
            $this->db->update('ip_quote_amounts', $data);
        } else {
            $this->db->insert('ip_quote_amounts', $data);
        }

        $this->calculate_quote_taxes($quoteId);
    }
    ...
    public function calculate_quote_taxes($quote_id)
    {
        ...
        if ($quote_tax_rates)
        {
            ...
            foreach ($quote_tax_rates as $quote_tax_rate)
            {
                ...
                $db_array = array('quote_tax_rate_amount' => $quote_tax_rate_amount);
            ...
            $db_array = array('quote_total' => $quote_total);
        ...
        else
        {
            ...
            $db_array = array('quote_tax_total' => '0.00');
        ...
        switch ($period)
        {
            case 'month' :
                return $this->db->query("
					...
					AND YEAR(quote_date_created) = YEAR(NOW())
					AND company_id=$company_id)")->row()->total_quoted;
            case 'last_month' :
                return $this->db->query("
					SELECT SUM(quote_total) AS total_quoted
					FROM ip_quote_amounts
					...
					AND YEAR(quote_date_created) = YEAR(NOW() - INTERVAL 1 MONTH)
					AND company_id=$company_id)")->row()->total_quoted;
            case 'year' :
                return $this->db->query("
					SELECT SUM(quote_total) AS total_quoted
					FROM ip_quote_amounts
					WHERE quote_id IN
					(SELECT quote_id FROM ip_quotes WHERE YEAR(quote_date_created) = YEAR(NOW())
					AND company_id=$company_id)")->row()->total_quoted;
            case 'last_year' :
                return $this->db->query("
					SELECT SUM(quote_total) AS total_quoted
					FROM ip_quote_amounts
					WHERE quote_id IN
					(SELECT quote_id FROM ip_quotes WHERE     YEAR(quote_date_created) = YEAR(NOW() - INTERVAL 1 YEAR)
					                                      AND company_id=$company_id)")->row()->total_quoted;
            default :
                return $this->db->query("SELECT SUM(quote_total) AS total_quoted FROM ip_quote_amounts WHERE ip_quotes.quote_id IN (SELECT quote_id from ip_quotes WHERE ip_quotes.company_id=$company_id)")->row()->total_quoted;
        }
    }
        ...
        switch ($period)
        {
            ...
            case 'this-month' :
                $results = $this->db->query("
                    ...
                    WHERE ip_quotes.quote_id IN (SELECT quote_id from ip_quotes WHERE ip_quotes.company_id=$company_id)
