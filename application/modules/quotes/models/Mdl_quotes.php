...
class Mdl_quotes extends Response_Model
{
    ...
    public function validation_rules()
    {
        return array(
            ...
            'company_id' => array('field' => 'company_id'),
    ...
    public function get_date_due($quote_date_created)
    {
        ...
        if ($this->Mdl_settings->setting('quotes_expire_after') == '')
            $set = 10;
else
            $set = $this->Mdl_settings->setting('quotes_expire_after');
        $quote_date_expires->add(new DateInterval('P' . $set . 'D'));
    ...
    {
        $this->filter_where_in('quote_status_id', array(
            2,
            3
        ));
        return $this;
    ...
    {
        $this->filter_where('ip_quotes.supplier_id', $supplier_id);
        return $this;
    }

    public function approve_quote_by_key($quote_url_key)
    {
        $this->db->where_in('quote_status_id', array(
            2,
            3
        ));
