...
function xml_invoice($params, $from_date, $to_date)
{
    $CI = &get_instance();
    ...
    foreach ($invoices as $invoice) {
        ...
        if ($invoice->client_swift == '') {
            $alerts .= sprintf(
                lang('set_swift'),
                $invoice->client_name,
                site_url('clients/form/' . $invoice->client_id)
            ) . '<br>';
        ...
        if ($invoice->client_iban == '') {
            $alerts .= sprintf(lang('set_iban'), $invoice->client_name,
                    site_url('clients/form/' . $invoice->client_id)) . '<br>';
            $check = false;
            $alertsCount++;
        }
        if ($invoice->company_code == '') {
            $alerts .= sprintf(lang('set_company_code_xml_error'), $invoice->client_name,
                    site_url('clients/form/' . $invoice->client_id)) . '<br>';
            $check = false;
            $alertsCount++;
        }
        if ($invoice->client_address_1 == '') {
            $alerts .= sprintf(lang('set_postal_address1_xml_error'), $invoice->client_name,
                    site_url('clients/form/' . $invoice->client_id)) . '<br>';
            $check = false;
            $alertsCount++;
        }
        if ($invoice->client_city == '') {
            $alerts .= sprintf(lang('set_city_xml_error'), $invoice->client_name,
                    site_url('clients/form/' . $invoice->client_id)) . '<br>';
            $check = false;
            $alertsCount++;
        }
        if ($invoice->company_iban == '') {
            $alerts .= lang('set_company_iban_xml_error') . '<br>';
            $check = false;
            $alertsCount++;
        }
    }
...
function gen_InvoiceInformation($xml, $invoice)
{
    ...
    $payment_meth = $CI->Mdl_payment_methods->filter_where('ip_payment_methods.company_id',
        $CI->session->userdata('company_id'))
        ->get()
        ->result();
...
function gen_PaymentInfo($xml, $invoice)
{
    $CI = &get_instance();
    $CI->load->model('settings/Mdl_settings');
    $xmlPaymentInfo = $xml->createElement("PaymentInfo");

    $xmlPayData = $xml->createElement("Currency");
    $xmlPayData->nodeValue = 'EUR';
    $xmlPaymentInfo->appendChild($xmlPayData);

    $xmlPayData = $xml->createElement("PaymentDescription");
    $xmlPayData->nodeValue = 'Invoice ' . $invoice->invoice_number;
    $xmlPaymentInfo->appendChild($xmlPayData);

    $xmlPayData = $xml->createElement("Payable");
    $xmlPayData->nodeValue = 'YES';
    $xmlPaymentInfo->appendChild($xmlPayData);

    $xmlPayData = $xml->createElement("PayDueDate");
    $xmlPayData->nodeValue = $invoice->invoice_date_due;
    $xmlPaymentInfo->appendChild($xmlPayData);

    $xmlPayData = $xml->createElement("PaymentTotalSum");
    $xmlPayData->nodeValue = $invoice->invoice_total;
    $xmlPaymentInfo->appendChild($xmlPayData);

    $xmlPayData = $xml->createElement("PaymentId");
    $xmlPayData->nodeValue = $invoice->invoice_number;
    $xmlPaymentInfo->appendChild($xmlPayData);

    $xmlPayData = $xml->createElement("PayToAccount");
    $xmlPayData->nodeValue = $invoice->company_iban;
    $xmlPaymentInfo->appendChild($xmlPayData);

    $xmlPayData = $xml->createElement("PayToName");
    $xmlPayData->nodeValue = $invoice->company_name;
    $xmlPaymentInfo->appendChild($xmlPayData);
